...
class DumpObject
{
    ...
    private string _Dump(object d, int level, bool displayType)
    {
        ...
        else if (d is IDictionary)
        {
            ...
            foreach (object key in dict.Keys)
            {
                ...
                Type fType = dict[key].GetType();
                dumpText += IsPrintValueNow(fType) ? dict[key].ToString() : _Dump(dict[key], level + 1, valueVerbose);
                dumpText += "," + CRLF;
            }
        ...
        else if (d is IList)
        {
            ...
            {
                dumpText += BuildIndent(level + 1);
                Type fType = item.GetType();
                dumpText += IsPrintValueNow(fType) ? item.ToString() : _Dump(item, level + 1, valueVerbose);
                dumpText += "," + CRLF;
            }
            dumpText += BuildIndent(level);
            dumpText += "]";
        ...
        else
        {
            ...
            foreach (FieldInfo dFieldInfo in dType.GetFields())
            {
                ...
                dumpText += IsPrintValueNow(fType) ? val.ToString() : _Dump(val, level + 1, valueVerbose);
                dumpText += ";" + CRLF;
            }
    ...
    private bool IsPrintValueNow(Type fType)
    {
        return fType.IsEnum || fType == typeof(Vector2) || fType == typeof(Vector3) || fType == typeof(Vector4) || fType == typeof(Quaternion);
    }
